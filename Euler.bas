...
Call MonteCarloEuler(Sheet, s0, a, div, vol, steps, sims, T)

End Sub

Sub MonteCarloEuler(Sheet As String, s0 As Double, a As Double, div As Double, vol As Double, steps As Integer, sims As Integer, T As Double)
...
For i = 0 To sims - 1
    For j = 1 To steps
        ...
        SP(i, j) = SP(i, j - 1) + SP(i, j - 1) * (drift * (T / steps) + vol * Sqr(T / steps) * delta_WP)
