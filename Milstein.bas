...
Call MonteCarloMilstein(Sheet, s0, a, div, vol, steps, sims, T)

End Sub
Sub MonteCarloMilstein(Sheet As String, s0 As Double, a As Double, div As Double, vol As Double, steps As Integer, sims As Integer, T As Double)
...
For i = 0 To sims - 1
    For j = 1 To steps
        ...
        SP(i, j) = SP(i, j - 1) + SP(i, j - 1) * (drift * dt + vol_i * Sqr(dt) * delta_WP + 1 / 2 * vol_i * vol_ii * ((delta_WP) ^ 2 - dt))
